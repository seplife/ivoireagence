...
import { Upload, X, ImagePlus, LogIn, Loader2 } from "lucide-react";
...
export default function PublishListing() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. Save image references in property_images table
      if (imageUrls.length > 0) {
        ...
        if (imgError) {
          console.error("Image refs error:", imgError);
        }
      }
  ...
  const inputClass =
    "h-11 w-full rounded-xl bg-secondary px-3 text-sm text-foreground outline-none transition-all focus:ring-2 focus:ring-accent/20 placeholder:text-muted-foreground";
  const selectClass =
    "h-11 w-full rounded-xl bg-secondary px-3 text-sm text-foreground outline-none transition-all focus:ring-2 focus:ring-accent/20 appearance-none";
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container py-6">
        <div className="mx-auto max-w-2xl">
          <h1 className="mb-6 font-display text-2xl text-foreground">Publier une annonce</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="w-full rounded-xl bg-secondary px-3 py-2.5 text-sm text-foreground outline-none transition-all focus:ring-2 focus:ring-accent/20 placeholder:text-muted-foreground"
                    maxLength={2000}
                  />
                </div>
              </div>
            ...
            <div className="rounded-2xl bg-card p-6" style={{ boxShadow: "var(--shadow-sm)" }}>
              ...
              <p className="mb-3 text-xs text-muted-foreground">
                Ajoutez jusqu'à 8 photos (max 5 Mo chacune). La première photo sera la photo principale.
              </p>
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {previews.map((src, i) => (
                  <div key={i} className="group relative aspect-square overflow-hidden rounded-xl bg-secondary">
                    ...
                    {i === 0 && (
                      <span className="absolute bottom-1.5 left-1.5 rounded-md bg-primary/80 px-1.5 py-0.5 text-[10px] font-bold text-primary-foreground">
                        Principale
                      </span>
                    )}
                  </div>
            ...
            <div className="rounded-2xl bg-card p-6" style={{ boxShadow: "var(--shadow-sm)" }}>
              ...
              <p className="mb-3 text-xs text-muted-foreground">
                Ces informations seront affichées sur votre annonce.
              </p>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder={[profile?.first_name, profile?.last_name].filter(Boolean).join(" ") || "Votre nom"}
                    className={inputClass}
                    maxLength={100}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    placeholder={profile?.phone || "+225 XX XX XX XX XX"}
                    className={inputClass}
                    maxLength={20}
                  />
                </div>
            ...
            <button
              type="submit"
              disabled={submitting}
              className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-accent text-sm font-bold text-accent-foreground transition-all duration-150 hover:brightness-110 active:scale-[0.96] disabled:opacity-50"
            >
              {submitting ? (
                <>
